...
def quadrilateral_can_merge_region_coarse(a: Quadrilateral, b: Quadrilateral, discard_connection_gap = 3, font_size_ratio_tol = 0.7) -> bool :
    # check if text direction for both boxes are the same
    if a.assigned_direction != b.assigned_direction :
        return False
    # check if text rotation for both boxes are within threshold
    if abs(a.angle - b.angle) > 15 * np.pi / 180 :
        return False
    fs_a = a.font_size
    fs_b = b.font_size
    fs = min(fs_a, fs_b)
    # check if text font for both boxes are generally the same
